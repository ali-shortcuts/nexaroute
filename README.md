...
This package is **v0.5.3**: the v0.4 translation + dashboard core, now extended with a provider-incident intelligence layer and a hedging/cache/usage tier that few if any open gateways combine. The code is runnable and heavily tested, but no software can honestly be guaranteed to contain zero bugs.

## What v0.5.3 adds on top of v0.4
...
- **Quota intelligence from rate-limit headers**: common OpenAI and Anthropic limit/remaining/reset headers are observed on every upstream response. v0.5.2 tracks request and token windows separately and overlays **in-flight local reservations** on top of the latest provider-reported remaining quota. Data-plane upstream legs reserve one request plus a conservative token bound while waiting for fresh evidence; long streams retain the reservation until body completion when the provider omits fresh remaining headers. Routing pressure therefore reacts to effective headroom instead of letting concurrent requests pile onto one stale quota snapshot. v0.5.3 also makes remaining quota monotonic inside an active reset window, so a late out-of-order response cannot overwrite newer lower quota evidence with a stale higher value. Providers remain available as last resort because quota semantics vary.
...
```bash
docker build -t nexaroute:0.5.3 .
...
The supported path is strong, but v0.5.3 is **not*** a universal implementation of every LLM protocol. Native OpenAI Responses, Gemini native `generateContent`, Bedrock, Vertex AI, Azure-specific deployment semantics, embeddings/rerank, encrypted-at-rest secret vaults, distributed state, invoice-perfect cost optimization or hard budget enforcement, and full internet-facing RBAC/CSRF hardening are not implemented.
...
before treating v0.5.3 as production infrastructure.
