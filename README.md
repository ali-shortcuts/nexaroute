...
The supported path is strong, but v0.5.2 is **not*** a universal implementation of every LLM protocol. Bedrock, Vertex AI, Azure-specific deployment semantics, embeddings/rerank, encrypted-at-rest secret vaults, distributed state, invoice-perfect cost optimization or hard budget enforcement, and full internet-facing RBAC/CSRF hardening are not implemented. (This branch additionally ships OpenAI Responses ingress/streaming and a Gemini upstream; see `CHANGELOG.md` Unreleased.)
