...
- Burst capacity for parallel-agent traffic (see `docs/RESILIENCE.md`): a bounded admission queue (`routing.admission_queue_timeout_ms`, default 5000ms) absorbs bursts bigger than `max_inflight_requests` instead of rejecting instantly; per-attempt provider queue timeouts (`routing.provider_queue_timeout_ms`, default 10000ms) spill saturated first-choice providers over to the next candidate immediately (no backoff pause, no health poisoning, no retry-budget spend — a spill is a redirect, not a retry), and all-saturated requests fail fast with 503 + `Retry-After: 1`; session pins yield to least-pressure ordering while the pinned provider is full so one shared session key cannot hotspot a deployment. Waits and spills are visible as `nexaroute_admission_waits_total` / `nexaroute_provider_saturated_total` metrics and `gateway_overloaded` / `provider_saturated` events.
