...
- **Responses streaming** (`stream:true` on `/v1/responses`): upstream SSE is normalized through the `compat/stream` decoders and rendered as Responses events (`response.created` → output item/part deltas → `response.completed` with real usage); function-call items stream with `{}`-padded arguments, length cutoffs report `incomplete`, and mid-stream truncation surfaces `response.failed`. Anthropic upstreams now receive a valid Canonical→Anthropic payload (previously an OpenAI-shaped body was posted to `/v1/messages`); OpenAI upstreams get `stream_options include_usage` with a one-shot stripped retry.
...
- **Hedged race latency**: the losing leg is now cancelled the moment the winner is decided instead of being awaited. Previously a slow loser held the client-visible response until it answered (measured: a 300ms primary win returned to the client at 3.05s with a 3s loser). The loser's context is cancelled immediately and its body is closed off the request path, so it cannot burn quota or hold a concurrency slot either. If the primary answers with a status the caller would fail over from, the already-in-flight hedge leg is still awaited and preferred, so failover is not regressed. A hedge is also no longer launched once the primary has settled.
- **Provider presets in the Web UI**: the admin API serializes presets as `{id, name, base_url, auth_mode, ...}` while the dashboard read `key`/`label`, so every built-in preset was dropped and only a stale hardcoded list was offered. The full catalog (SiliconFlow CN, Qwen US/International, NVIDIA NIM, SambaNova, Cerebras, MiniMax, Moonshot, Hugging Face, GitHub Models, LM Studio, vLLM, …) is now selectable and its paths/auth apply.
- **Provider "Edit" button**: the click handler was attached with a `querySelector` call on a NodeList, which threw and aborted rendering, so the edit modal never opened.
- **One release version**: the HTTP metadata (`/api/hello`, `/version`, admin snapshot) now reports `internal/buildinfo.Version`, and `TestVersionMatchesBuildinfo` pins the CLI literal to it, so the two can no longer disagree. They did: the CLI and logs said 0.5.0 while the dashboard showed `v0.4.1`.
- **Dead duplicate helpers removed**: `applyProviderHeaders` (duplicate of the provider adapter's header/auth logic), `proxyValidatedJSONResponse` (superseded by the inlined passthrough path) and `hardCooldownStatus` had no call sites.
- **`scripts/smoke-v05.sh` was unusable and orphaned**: it invoked the binary through a path relative to `scripts/`, so it exited immediately while leaking its fake upstream, and it could hang any wrapper because the child kept stdout open. It now resolves the binary and architecture, waits for both listeners, bounds every request with timeouts, asserts cache/hedging/metrics/health/UI with actionable failures, cleans up every process it starts, and runs at the end of `scripts/verify.sh` (therefore in CI).
- UI: provider authentication offers **Auto (by provider type)** as specified, and an auto-configured provider is no longer silently rewritten to bearer when edited.
- Docs: `SECURITY.md`, `docs/KNOWN_GAPS.md`, `docs/CONFIGURATION.md`, `docs/COMPATIBILITY.md` and `docs/PROVIDER_UI_SPEC.md` are titled for v0.5; the security boundary now documents the opt-in `client_auth` data-plane keys instead of claiming the plane has no built-in auth; the reasoning/thinking gap now describes the actual capability-gated routing and `reasoning_effort` translation instead of claiming routing is constrained to the matching native protocol; README Docker tags no longer pin 0.4.1.
