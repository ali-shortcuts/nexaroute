...
- Burst capacity for parallel-agent traffic (see `docs/RESILIENCE.md`): a bounded admission queue (`routing.admission_queue_timeout_ms`, default `0` = wait while the client is connected) absorbs Claude Code sub-agent bursts bigger than `max_inflight_requests` (default 256) instead of 503ing a live caller; per-attempt provider queue timeouts (`routing.provider_queue_timeout_ms`, default `0` = wait the route budget so a single-provider Claude Code setup queues extra sub-agents; a positive value spills to the next candidate) (no backoff pause, no health poisoning, no retry-budget spend — a spill is a redirect, not a retry), and all-saturated requests fail fast with 503 + `Retry-After: 1`; session pins yield to least-pressure ordering while the pinned provider is full so one shared session key cannot hotspot a deployment. Provider concurrency default 64, stream idle 600s (thinking pauses), 32MiB request bodies and SSE lines so high-token generations and large tool payloads are not cut off; upstream HTTP transport does not cap connections at the semaphore; listener ReadTimeout 5 minutes with no WriteTimeout. Waits and spills are visible as `nexaroute_admission_waits_total` / `nexaroute_provider_saturated_total` metrics and `gateway_overloaded` / `provider_saturated` events.
