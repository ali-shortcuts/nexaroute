...
func (m *Manager) now() time.Time {
	if m != nil {
		if v := m.nowFn.Load(); v != nil {
			if fn, ok := v.(func() time.Time); ok && fn != nil {
				return fn()
			}
		}
	}
...
// SetNow replaces the time source. Tests use this with a fake clock so a
// 30-minute cooldown can be advanced without sleeping.
...
func (m *Manager) RecordFailure(id, errMsg string, latency time.Duration) {
	...
	if s.Status == HalfOpen || s.ConsecutiveFailures >= m.threshold {
		s.Status = Cooldown
		s.CooldownUntil = m.now().Add(m.cooldown)
	} else {
		s.Status = Degraded
