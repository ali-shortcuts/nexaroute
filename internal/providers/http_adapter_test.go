...
func TestStaleRemainingHeaderReleasesReservationWithoutRaisingBudget(t *testing.T) {
	...
	h := http.Header{}
	h.Set("x-ratelimit-remaining-requests", "6") // stale/out-of-order higher value
	obs := a.observeRateLimitHeaders(h)
	if !obs.RequestRemaining {
		t.Fatal("remaining header should still count as quota evidence for reservation release")
	}
