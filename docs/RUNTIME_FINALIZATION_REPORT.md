...
- Added and ran an offline release-style installer E2E using local assets/curl/uname shims: amd64 and arm64 asset selection, checksum verification/rejection, command on PATH, executable install, no Go requirement, config directory creation, upgrade config preservation, and safe failure without replacing the prior binary pass. It does not run the real gateway binary; that remains incomplete.
...
| `scripts/test-install.sh` offline release-style E2E | PASS: amd64/arm64 asset selection, checksum rejection, no-Go install, PATH command, config dir and upgrade preservation |
