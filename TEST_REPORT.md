...
- burst-capacity fault injection: admission bursts bigger than the global limit queue and drain (no instant 503s) while a tiny queue timeout still rejects fast with 503 + `Retry-After`; saturated first-choice providers spill to the next candidate without health poisoning, and all-saturated requests fail fast with 503 + `Retry-After`; saturated hedge losers record nothing; session pins yield to idle providers at pressure ≥ 1;
